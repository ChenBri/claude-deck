...
| 59 | Effort dial | A second SR16-family rotary switch (same part as the session selector, decision 21), 4 of its positions wired: LOW/MED/HIGH/MAX. Added 2026-09-22 once the display upgrade's wider deck left real room for it. Wired to MCP23017 #2's spares, no new IC. The dial only sends a labelled action to the daemon; what the daemon actually does with it (`effortSelect` in `daemon/src/actions/`) is a placeholder hotkey in the same spirit as the existing `planMode()` - there's no confirmed way yet to change Claude Code's reasoning effort from an external keystroke, so this needs tuning to whatever that turns out to be locally. |
